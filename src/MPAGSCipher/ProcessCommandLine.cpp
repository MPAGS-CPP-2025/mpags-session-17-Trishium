...
ProgramSettings processCommandLine(const std::vector<std::string>& cmdLineArgs)
{
        ...
        if (cmdLineArgs[i] == "-h" || cmdLineArgs[i] == "--help") {
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                for (const auto& elem : arg) {
                    if (!std::isdigit(elem)) {
                        throw NonNumericArgument("--multi-cipher requires a positive integer argument, the supplied string (" + arg + ") could not be successfully converted");
                    }
                }
            ...
            if (i == nCmdLineArgs - 1) {
                throw MissingArgument("-c requires a cipher name argument");
                ...
                if (cmdLineArgs[i + 1] == "caesar") {
                    ...
                    throw CipherDoesNotExist("unknown cipher '" + cmdLineArgs[i + 1] + "' - expected 'caesar', 'playfair', or 'vigenere'");
                }
                ++i;
            }
        } else {
            ...
            // set the flag to indicate the error and terminate the loop
            throw UnknownArgument("unknown command line argument '" + cmdLineArgs[i] + "'");
        }
    }
    ...
    if (nTypes != nExpectedCiphers || nKeys != nExpectedCiphers) {
        throw UnexpectedQuantity("expected types and keys for " + std::to_string(nExpectedCiphers) + " ciphers, but received " + std::to_string(nTypes) + " types and " + std::to_string(nKeys) + " keys");
    }
